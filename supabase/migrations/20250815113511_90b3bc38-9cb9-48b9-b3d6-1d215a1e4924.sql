-- Función actualizada para validar compatibilidad de vehículo con presupuesto primario
...
BEGIN
  ...
  IF NEW.is_primary = true AND (OLD.is_primary IS NULL OR OLD.is_primary = false) THEN
    ...
    -- Determinar plazas esperadas según el modelo (misma lógica que en frontend)
    IF budget_info.model_name IS NOT NULL THEN
      IF LOWER(budget_info.model_name) LIKE '%neo s%' THEN
        expected_plazas := '2';
      ELSIF LOWER(budget_info.model_name) LIKE '%neo%' THEN
        expected_plazas := '3';
      END IF;
    END IF;
    ...
    -- Plazas (validación añadida)
    IF vehicle_info.plazas IS NOT NULL AND expected_plazas IS NOT NULL THEN
      IF vehicle_info.plazas != expected_plazas THEN
        -- Desasignar vehículo por incompatibilidad de plazas
        UPDATE public."NEW_Projects" 
        SET vehicle_id = NULL 
        WHERE id = NEW.project_id;
        ...
        RAISE NOTICE 'Vehículo desasignado: plazas no coinciden. Vehículo: % plazas, Modelo %: % plazas', 
          vehicle_info.plazas, budget_info.model_name, expected_plazas;
